...
void CAHP::relocate(uint8_t *loc, const Relocation &rel,
  ...
  case R_CAHP_PCREL_11: {
    checkInt(ctx, loc, static_cast<int64_t>(val), 11, rel);
  ...
  default:
    Err(ctx) << getErrorLoc(ctx, loc) << "unimplemented relocation: "
             << rel.type;
    return;
  }
}

void elf::setCAHPTargetInfo(Ctx &ctx) { ctx.target.reset(new CAHP(ctx)); }
